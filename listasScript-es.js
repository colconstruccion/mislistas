   ...
   function saveList() {
      const title = document.getElementById("title").value.trim() || `Lista sin titulo`;
      ...
      alert("Lista guardada en session.");
    }
  ...
  function openWorkspace() {
      ...
      if (lists.length === 0) {
        workspace.innerHTML = "<p>No hay listas guardadas.</p>";
        return;
      }

      let html = "<h4>Mis lisas guardadas</h4><table style='width: 100%; border-collapse: collapse;'>";
  ...
  function deleteList(index) {
    ...
    function createItemInputs(count, columns = 1) {
      ...
      for (let i = 1; i <= count; i++) {
        ...
        input.placeholder = 'Escriba el item';
    ...
    function updatePreview() {
      previewTitle.textContent = titleInput.value.trim() || 'Titulo de la lista...';
