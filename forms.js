document.addEventListener("DOMContentLoaded", () => {
  ...
  // Open My Forms panel
  // Toggle My Forms submenu
if (myFormsLink && myFormsSubmenu) {
  myFormsLink.addEventListener("click", (e) => {
    e.preventDefault();

    const isOpen = myFormsSubmenu.style.display === "block";
    myFormsSubmenu.style.display = isOpen ? "none" : "block";
  });
}

// Toggle My Logins panel
if (myLoginsLink && formsListPanel) {
    myLoginsLink.addEventListener("click", async (e) => {
      e.preventDefault();
      ...
      if (window.openVaultFlow) {
        const unlocked = await window.openVaultFlow();

        if (unlocked && window.loadUserForms) {
